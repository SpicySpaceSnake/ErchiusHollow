...
function init()
  ...
  self.effectDesatDistance = config.getParameter("effectDesatDistance")
...
function update(dt)
  ...
  erchiusCount = 500 + erchiusCount + (world.entityHasCountOfItem(entity.id(), "liquidfuel") or 0)
  erchiusCount = 500 + erchiusCount + (world.entityHasCountOfItem(entity.id(), "solidfuel") or 0)
  ...
  if monsterPosition then
    if not self.messaged then
      world.sendEntityMessage(entity.id(), "queueRadioMessage", "ehr_erchiussickness2")
      self.messaged = true
    end
	...
	local effectDesatDistance = interp.linear(erchiusRatio, self.effectDesatDistance[1], self.effectDesatDistance[2])
    local effectDesatDistanceRatio = 1 - math.min(1.0, math.max(0.0, vec2.mag(distance) / effectDesatDistance))

    self.saturation = math.floor(-self.desaturateAmount * effectDesatDistanceRatio)
